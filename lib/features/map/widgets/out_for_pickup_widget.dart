...
class _AcceptedTripWidgetState extends State<_AcceptedTripWidget> {
  ...
  void initState() {
    super.initState();
    // 1-second ticker to refresh the countdown/count-up display
    _ticker = Timer.periodic(const Duration(seconds: 1), (_) {
      if (mounted) setState(() {});
    });
    // 30-second backend sync for accurate fee data
    _syncTimer = Timer.periodic(const Duration(seconds: 30), (_) {
      final rc = Get.find<RideController>();
      if (rc.hasArrivedAtPickup && rc.tripDetail?.id != null) {
        rc.getLiveFees(rc.tripDetail!.id!);
      }
    });
    // Immediate fetch if already arrived (e.g. widget rebuild / app restart)
  ...
  /// Build the dynamic waiting-time box shown after driver arrives at pickup.
  Widget _buildWaitingBox(BuildContext context, RideController rc) {
    final DateTime? arrived = rc.arrivedAt;
    final int graceSec = (rc.gracePeriodMinutes * 60).toInt();
    final double feePerMin = rc.waitingFeePerMin;

    // If arrivedAt is not yet known, show a brief loading indicator
    if (arrived == null) {
    ...
    final int elapsedSec =
        DateTime.now().difference(arrived).inSeconds.clamp(0, 86400);
    final int graceRemainSec = graceSec - elapsedSec;
    final double billableMinutes =
        graceRemainSec < 0 ? (-graceRemainSec) / 60 : 0;
    final double apiWaitingFee = rc.tripDetail?.waitingFee ?? 0;
    final double displayedWaitingFee =
        apiWaitingFee > 0 ? apiWaitingFee : (billableMinutes * feePerMin);
    ...
    return Container(
      ...
      child: graceRemainSec > 0
          // ── Phase 1: Grace period countdown ──
          ? Row(
              ...
              children: [
                ...
                Text(
                  '${'free_waiting'.tr}: ${DateConverter.formatDurationHms(graceRemainSec)}',
          ...
          // ── Phase 2: Billable waiting time + fee ──
          : Row(
              ...
              children: [
                Row(
                  children: [
                    ...
                    Text(
                      '${'waiting_time'.tr}: ${DateConverter.formatDurationHms(-graceRemainSec)}',
                      style: textMedium.copyWith(
                        color: Theme.of(context).primaryColor,
                        fontSize: Dimensions.fontSizeSmall,
                      ),
                ...
                Text(
                  PriceConverter.convertPrice(context, displayedWaitingFee),
                  style: textMedium.copyWith(
                    color: Theme.of(context).primaryColor,
                    fontSize: Dimensions.fontSizeSmall,
                  ),
      ...
      return GetBuilder<RideController>(builder: (rideController) {
        ...
        return Padding(
          ...
          child: Column(children: [
            ...
            const SizedBox(height: Dimensions.paddingSizeDefault),
            if (!rideController.hasArrivedAtPickup)
              Padding(
                padding: const EdgeInsets.symmetric(
                    horizontal: Dimensions.paddingSizeDefault),
                child: ButtonWidget(
                  buttonText: 'i_have_arrived_at_pickup'.tr,
                  showBorder: true,
                  transparent: true,
                  borderColor: Theme.of(context).primaryColor,
                  ...
                  onPressed: () {
                    rideController
                        .markArrivedAtPickup(rideController.tripDetail!.id!);
                  },
                ),
              )
            else
              Padding(
                padding: const EdgeInsets.symmetric(
                    horizontal: Dimensions.paddingSizeDefault),
                child: _buildWaitingBox(context, rideController),
              ),
            const SizedBox(height: Dimensions.paddingSizeSmall),
            if (!(Get.find<SplashController>().config?.otpConfirmationForTrip ??
                false)) ...[
              rideController.isPinVerificationLoading
                  ...
                  : Padding(
                      ...
                      child: SliderButton(
                        action: () {
                          if (!rideController.hasArrivedAtPickup) {
                            showCustomSnackBar('please_mark_arrival_first'.tr,
                                isError: true);
                            return;
                          }
                          rideController
